...
class Gallery(StatusModel, TimeStampedModel):
    ...
    def get_absolute_url(self):
        return ('gallery:gallery_details', [], {'slug': self.slug})
...
class Photo(TimeStampedModel):
    ...
    def save(self, *args, **kwargs):
        ...
        try:
            ...
            if raw_exif:
                self.exif = {ExifTags.TAGS[k]: sanitize_exif_value(k, v) for k, v in raw_exif.items() if
                             k in ExifTags.TAGS}
        except Exception:
            pass
    ...
    @models.permalink
    def get_absolute_url(self):
        return ('gallery:photo_details', [], {'pk': self.pk})
...
class Video(TimeStampedModel):
    gallery = models.ForeignKey(Gallery, null=True, related_name='videos', verbose_name=_("gallery"),
                                on_delete=models.CASCADE)
    author = models.ForeignKey(settings.AUTH_USER_MODEL, editable=False, verbose_name=_("author"),
                               on_delete=models.CASCADE)
    title = models.CharField(_("title"), max_length=255)
